...
export default factories.createCoreController('api::progress.progress', ({ strapi }) => ({
  ...
  //   try {
  //     const enrollments = await strapi.db.query('api::enrollment.enrollment').findMany({
  //       populate: ['student', 'course', 'course.lessons', 'course.instructor', 'course.user'],
  //     });
  ...
  //     // Filter enrollments to own courses if instructor
  //     const targetEnrollments = isInstructor
  //       ? enrollments.filter((en: any) => {
  //           const ownerId = en.course?.instructor?.id || en.course?.user?.id;
  ...
  async getAllStudentProgress(ctx) {
    ...
    try {
      // 1. Populate student and student's role to exclude non-students
      const enrollments = await strapi.db.query('api::enrollment.enrollment').findMany({
      ...
      // 2. Filter out non-students (Admins, Instructors, Content Managers)
      const validStudentEnrollments = enrollments.filter((en: any) => {
        ...
        const isActualStudent = !studentRole.includes('admin') && !studentRole.includes('instructor') && !studentRole.includes('manager') && !studentRole.includes('content');
        return isActualStudent;
      });

      // 3. If Instructor, filter to enrollments for their own courses only
      const targetEnrollments = isInstructor
