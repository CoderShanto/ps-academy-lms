...
export default factories.createCoreController('api::progress.progress', ({ strapi }) => ({
  async getCourseProgress(ctx) {
    ...
    try {
      // 1. Fetch the course with its lessons
      ...
      // 2. Query completed progress for this student's lessons in this course
      ...
      const completedLessonIds = completedProgress.map((p: any) => p.lesson?.id || p.lesson?.documentId).filter(Boolean);
