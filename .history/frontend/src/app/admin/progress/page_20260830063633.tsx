...
export default function StudentProgressOverviewPage() {
  ...
  useEffect(() => {
    if (!authLoading) {
      ...
      if (!user) {
        router.push('/login');
      } else if (!roleName.includes('admin') && !roleName.includes('instructor') && !roleName.includes('content') && !roleName.includes('manager')) {
        router.push('/student');
      } else {
        fetchOverview();
      }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-8">
        <Link href="/admin" className="inline-flex items-center text-sm font-medium text-gray-400 hover:text-white mb-4">
          <ArrowLeft className="h-4 w-4 mr-1.5" /> Back to Admin Overview
        </Link>
        <h1 className="text-3xl font-bold text-gray-900">Student Learning Progress</h1>
        <p className="text-gray-600 mt-1">Live tracking of course completions, enrolled students, and lesson milestones</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-gray-100 flex items-center justify-between bg-gray-50">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="font-bold text-gray-900 text-base">All Enrollments & Completion Rates</h2>
          </div>
