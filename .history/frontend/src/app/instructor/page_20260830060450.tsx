...
import { PlusCircle, BookOpen, Video, Trash2, HelpCircle } from 'lucide-react';
...
export default function CourseManagementStudio() {
  ...
  // Handle Course Creation (Without invalid 'instructor' key)
// Helper to generate a URL-friendly slug
  ...
  // Handle Course Creation with auto-generated slug
  ...
  // Handle Lesson & Quiz Creation with safe numeric 'order'
  const handleCreateLesson = async (e: React.FormEvent) => {
    ...
    try {
      // Find current lessons count to calculate numeric order
      const currentCourse = courses.find(
        (c) => String(c.id) === String(selectedCourseId) || c.documentId === String(selectedCourseId)
      );
      const nextOrderNumber = (currentCourse?.lessons?.length || 0) + 1;

      // 1. Create Lesson with a valid numeric order
      ...
      // 2. Optionally create Question & Quiz
      ...
      alert('Lesson and Quiz added successfully!');
      await fetchCourses();
    } catch (err: unknown) {
      if (isAxiosError(err)) {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-1">
          Create and manage courses, video lessons, and interactive auto-graded quizzes
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
            ...
            <div className="space-y-3 max-h-[400px] overflow-y-auto">
              {courses.map((c) => (
                <div
                  key={c.id}
                  className="p-3.5 rounded-xl border border-gray-100 bg-gray-50 flex items-center justify-between"
                >
        ...
        {/* Right 2 Columns: Lesson & Quiz Builder Form */}
        <div className="lg:col-span-2">
          <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-gray-200">
            <h2 className="text-xl font-bold text-gray-900 mb-6 flex items-center">
              <Video className="h-6 w-6 mr-2 text-indigo-600" /> Add Lesson & Quiz to Course
            </h2>
            ...
            <form onSubmit={handleCreateLesson} className="space-y-5">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2.5 bg-white text-gray-900 border border-gray-300 rounded-lg text-sm font-medium focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                >
                  {courses.map((c) => (
              ...
              >
                {creatingLesson ? 'Adding to Curriculum...' : 'Add Lesson to Course'}
              </button>
            </form>
          </div>
        </div>
