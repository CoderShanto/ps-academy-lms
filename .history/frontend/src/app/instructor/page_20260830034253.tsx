...
export default function InstructorStudioPage() {
  ...
  const handleCreateCourse = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // In Strapi 5, pass instructor relation by ID/documentId if available
      if (user?.id) {
      ...
      if (isAxiosError(err)) {
        // Fallback retry without instructor relation key if relation is not configured
        try {
          await api.post('/courses', { data: { title, description } });
          setTitle('');
          setDescription('');
          fetchInstructorCourses();
          return;
