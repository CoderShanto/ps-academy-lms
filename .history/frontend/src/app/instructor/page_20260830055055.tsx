...
export default function CourseManagementStudio() {
  ...
  const fetchCourses = useCallback(async () => {
    try {
      ...
      if (data.data?.length > 0 && !selectedCourseId) {
        setSelectedCourseId(data.data[0].documentId || data.data[0].id);
      }
    } catch (err) {
  ...
  const handleCreateLesson = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const createdLessonId = lessonRes.data?.data?.id || lessonRes.data?.data?.documentId;

      // 2. Optionally create and link Quiz
      if (attachQuiz && questionText.trim() && createdLessonId) {
        ...
        const createdQuestionId = qRes.data?.data?.id || qRes.data?.data?.documentId;
      ...
      alert('Lesson and curriculum item created successfully!');
      await fetchCourses();
    } catch (err: unknown) {
      if (isAxiosError(err)) {
