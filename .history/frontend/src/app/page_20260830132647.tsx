...
const STATS = [
  { label: 'Roles enforced server-side', value: '4', icon: Users },
  { label: 'Client-side answer leaks', value: '0', icon: ShieldCheck },
  { label: 'Quiz grading', value: 'Instant', icon: Zap },
...
const FEATURES = [
  {
    ...
    desc: 'Structured curriculum combining video and text content, ordered by course creators with built-in progress tracking.',
  },
  {
    icon: GraduationCap,
    color: '#34D399',
    title: 'Automated Assessments',
    desc: 'Quizzes graded server-side with instant feedback — answer keys never reach the client.',
  },
  {
    icon: ShieldCheck,
    color: '#C084FC',
    title: 'Role-Based Access Control',
    desc: 'Four-tier permission system across Admin, Instructor, Content Manager, and Student, enforced on the backend.',
  },
  {
    icon: BarChart3,
    color: '#FBBF24',
    title: 'Real-Time Progress Tracking',
    desc: 'Live dashboards for learners and administrators, updated as courses and quizzes are completed.',
  },
  {
    icon: Target,
    color: '#F472B6',
    title: 'Role-Specific Dashboards',
    desc: 'Each role lands on a workspace scoped to exactly what they need — no clutter, no unused controls.',
  },
  {
    icon: Layers,
    color: '#60A5FA',
    title: 'Modular Course Structure',
    desc: 'Courses are broken into modules and lessons, making content easy to organize, reorder, and extend.',
  },
];

const STACK = ['Next.js', 'Strapi', 'Tailwind CSS', 'TypeScript', 'REST API', 'JWT Auth'];
...
export default function Home() {
  ...
  return (
    <div className="relative bg-[#05060A]">
      ...
      {/* HERO */}
      <section className="relative max-w-7xl mx-auto px-6 pt-24 pb-16 grid lg:grid-cols-[1.05fr_0.95fr] gap-16 items-center">
        <div>
        ...
        <div className="relative">
          ...
          <div className="relative rounded-2xl border border-white/10 bg-[#0B0D14]/90 backdrop-blur-xl shadow-2xl overflow-hidden">
            ...
            <div className="flex items-center justify-between px-5 py-3 border-t border-white/10 bg-white/[0.02]">
              <span className="font-mono text-[10px] text-white/40">access resolved automatically per role</span>
      ...
      {/* STATS STRIP */}
      <section className="relative max-w-7xl mx-auto px-6 pb-4">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 border-t border-white/5 pt-10">
          {STATS.map((s) => (
            <div key={s.label} className="rounded-xl border border-white/10 bg-white/[0.03] backdrop-blur p-5">
              <s.icon className="h-4 w-4 text-[#22D3EE] mb-3" />
      ...
      {/* ROLES */}
      <section className="relative max-w-7xl mx-auto px-6 py-20 border-t border-white/5 mt-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {ROLES.map((r) => (
            ...
            >
              <div className="w-10 h-10 rounded-lg flex items-center justify-center mb-5" style={{ background: `${r.color}1A` }}>
      ...
      <section className="relative max-w-7xl mx-auto px-6 py-20 border-t border-white/5">
        <div className="max-w-2xl mb-14">
          ...
          <h2 className="mt-3 text-3xl sm:text-4xl font-bold text-white tracking-tight">From enrollment to mastery.</h2>
        </div>
      ...
      <section className="relative max-w-7xl mx-auto px-6 py-20 border-t border-white/5">
        <div className="max-w-2xl mb-12">
          ...
          <h2 className="mt-3 text-3xl sm:text-4xl font-bold text-white tracking-tight">Designed for real classrooms.</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {FEATURES.map((f) => (
            <div key={f.title} className="group rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur p-7 hover:bg-white/[0.06] hover:border-white/20 transition">
              <div className="w-11 h-11 rounded-xl flex items-center justify-center mb-5" style={{ background: `${f.color}1A` }}>
      ...
      {/* TECH STACK — honest social proof for a technical audience */}
      <section className="relative max-w-7xl mx-auto px-6 py-20 border-t border-white/5">
        <div className="max-w-2xl mb-10">
          <span className="font-mono text-[11px] tracking-wide text-[#34D399]">05 · under the hood</span>
          <h2 className="mt-3 text-3xl sm:text-4xl font-bold text-white tracking-tight">Built on a modern stack.</h2>
          <p className="mt-4 text-white/50 leading-relaxed">
            A production-style architecture — API-driven backend, typed frontend,
            and permissions enforced where it matters.
          </p>
        </div>
        <div className="flex flex-wrap gap-3">
          {STACK.map((t) => (
            <span key={t} className="font-mono text-sm text-white/70 border border-white/10 bg-white/[0.03] rounded-lg px-4 py-2">
              {t}
            </span>
          ))}
        </div>
      ...
      {/* CLOSING CTA */}
      <section className="relative max-w-7xl mx-auto px-6 pb-24 pt-8 border-t border-white/5">
        <div className="relative rounded-3xl border border-white/10 bg-white/[0.03] backdrop-blur px-10 py-16 text-center overflow-hidden">
          ...
          <h2 className="relative text-3xl sm:text-4xl font-bold text-white tracking-tight mb-4">Ready to start learning?</h2>
      ...
      <footer className="relative border-t border-white/5 py-12">
        <div className="max-w-7xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <span className="text-white/40 text-sm">PS Academy — a Next.js + Strapi learning platform</span>
          <span className="font-mono text-xs text-white/25">Final Year Design Project</span>
        </div>
      </footer>
      ...
      <style jsx>{`
        ...
        @keyframes wordIn { 0% { transform: translateY(100%); opacity: 0; } 100% { transform: translateY(0); opacity: 1; } }
      `}</style>
    </div>
  );
}