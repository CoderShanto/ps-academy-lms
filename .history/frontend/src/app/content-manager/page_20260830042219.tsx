...
export default function ContentManagerPage() {
  ...
  const handleCreateArticle = async (e: React.FormEvent) => {
    ...
    // Strict payload: only title and content
    const cleanPayload: { title: string; content: string; publishedAt?: string | null } = {
      title: title.trim(),
      content: content.trim(),
    };

    if (isPublished) {
      cleanPayload.publishedAt = new Date().toISOString();
    } else {
      cleanPayload.publishedAt = null;
    }

    try {
      await api.post('/blog-posts', { data: cleanPayload });
