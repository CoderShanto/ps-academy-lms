...
export default function ContentManagerPage() {
  ...
  const fetchArticles = useCallback(async () => {
    try {
      // In Strapi v5, status=draft query param retrieves both drafts and published entries for managers
      const { data } = await api.get('/blog-posts?status=draft&populate=*');
  ...
  const handleCreateArticle = async (e: React.FormEvent) => {
    ...
    // Strapi native draft vs publish payload
    ...
    try {
      ...
      if (isAxiosError(err)) {
        // Fallback retry with just title and content
        try {
          ...
          if (isAxiosError(retryErr)) {
            setError(retryErr.response?.data?.error?.message || 'Failed to create article');
          }
        }
      } else {
