...
export default function PublicCourseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const isStudent = roleName.includes('student') || (!roleName.includes('admin') && !roleName.includes('instructor') && !roleName.includes('manager') && !roleName.includes('content'));
  ...
  const handleEnroll = async () => {
    ...
    if (!isStudent) {
      alert('Only student accounts are permitted to enroll in courses.');
      return;
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div>
          <div className="bg-white rounded-3xl p-6 border border-gray-200 shadow-sm sticky top-6">
            ...
            {/* Case 1: Staff Roles (Admin, Content Manager, Instructor) - Cannot Enroll */}
            {isStaffRole ? (
