...
export default function StudentCourseViewerPage() {
  ...
  const handleMarkComplete = async () => {
    ...
    try {
      ...
      const updatedIds = Array.from(new Set([...completedLessonIds, selectedLesson.id, selectedLesson.documentId].filter(Boolean)));
      setCompletedLessonIds(updatedIds as (string | number)[]);
      
      const newPercentage = data.meta?.percentage ?? Math.min(100, Math.round((updatedIds.length / (course.lessons?.length || 1)) * 100));
