...
export default function CourseDetailsPage() {
  ...
  useEffect(() => {
    const fetchCourseAndStatus = async () => {
      try {
        // Try fetching course by ID / documentId
          ...
          try {
            ...
            const isUserEnrolled = (myCoursesRes.data || []).some(
              (c: { id: number; documentId?: string }) =>
                c.id === Number(id) || (courseData?.documentId && c.documentId === courseData.documentId)
            );
  ...
  if (!course) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-gray-500 mt-2">Please ensure this course is Published in the Strapi Admin panel.</p>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="bg-indigo-600 p-8 text-white">
          ...
          <div className="mt-6 flex items-center space-x-4">
            {enrolled ? (
              <button
                onClick={() => router.push(`/student/courses/${course.id || id}`)}
