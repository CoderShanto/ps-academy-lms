...
export default function CourseDetailsPage() {
  ...
  useEffect(() => {
    const fetchCourseAndStatus = async () => {
      try {
        let fetchedCourse: CourseDetails | null = null;

        // Try direct Strapi 5 documentId lookup first
        try {
          const { data } = await api.get(`/courses/${id}?populate=*`);
          fetchedCourse = data.data;
        } catch {
          // Fallback: If numeric ID was passed in URL, filter by numeric ID
        ...
        if (user && fetchedCourse) {
          try {
            ...
            const isUserEnrolled = (myCoursesRes.data || []).some(
              (c: { id: number; documentId?: string }) =>
                c.id === fetchedCourse?.id || (fetchedCourse?.documentId && c.documentId === fetchedCourse.documentId)
            );
            setEnrolled(isUserEnrolled);
          } catch (enrollErr) {
            console.warn('Could not fetch enrollment status:', enrollErr);
          }
        }
      } catch (err: unknown) {
  ...
  if (!course) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-gray-500 mt-2">Please go back to the catalog and select an active course.</p>
      </div>
    );
  }
