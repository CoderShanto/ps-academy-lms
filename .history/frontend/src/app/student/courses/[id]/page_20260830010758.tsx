...
// Convert any YouTube link into a clean embed link
...
export default function StudentCourseViewerPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          ...
          if (data.data && data.data.length > 0) fetchedCourse = data.data[0];
        }
        ...
        if (fetchedCourse) {
          ...
          try {
            ...
            console.warn('Progress fetch warning:', progErr);
          }
        }
      } catch (err) {
  ...
  const handleMarkComplete = async () => {
    ...
    setMarking(true);
    try {
      const lessonTarget = selectedLesson.documentId || selectedLesson.id;
      ...
      if (!completedLessonIds.includes(currentId)) {
        setCompletedLessonIds((prev) => [...prev, currentId]);
      }
      setPercentage(data.meta?.percentage ?? percentage);
    } catch (err) {
  ...
  const isCurrentCompleted = selectedLesson
    ? completedLessonIds.includes(selectedLesson.id) || (selectedLesson.documentId && completedLessonIds.includes(selectedLesson.documentId))
    : false;
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
              <div className="flex items-center justify-between pb-4 border-b border-gray-100 mb-6">
                ...
                <button
                  ...
                  disabled={marking || Boolean(isCurrentCompleted)}
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            ...
            {course.quizzes && course.quizzes.length > 0 && (
              <div className="mt-6 pt-6 border-t border-gray-100">
                <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Course Quizzes</h4>
