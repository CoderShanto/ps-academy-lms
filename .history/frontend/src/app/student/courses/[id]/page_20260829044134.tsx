...
export default function StudentCourseViewerPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        // 1. Fetch course details with lessons and quizzes
        const { data: courseRes } = await api.get(`/courses/${id}?populate=*`);
        const courseData = courseRes.data;
        setCourse(courseData);

        const sortedLessons = (courseData.lessons || []).slice().sort(
        ...
        // 2. Fetch progress
        const { data: progRes } = await api.get(`/progress/course/${id}`);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Right 1 Column: Lesson Playlist & Quiz Menu */}
