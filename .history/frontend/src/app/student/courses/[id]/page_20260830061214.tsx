...
export default function StudentCoursePlayerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Check quiz relation either via mapped API or nested object
  const currentLessonKey = activeLesson?.id || activeLesson?.documentId || '';
  const linkedQuiz =
    (activeLesson && availableQuizzes[currentLessonKey]) ||
  ...
  const activeQuizId = linkedQuiz?.documentId || linkedQuiz?.id;
  const embedSrc = formatEmbedUrl(activeLesson?.videoUrl);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Left 2 Cols: Video Player & Lesson Details */}
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            ...
            {activeQuizId && (
              <div className="mt-6 p-5 rounded-xl bg-indigo-50 border border-indigo-200 flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <Award className="h-7 w-7 text-indigo-600 flex-shrink-0" />
                  <div>
                    <h4 className="font-bold text-gray-900 text-base">{linkedQuiz?.title || 'Lesson Quiz'}</h4>
                    <p className="text-xs text-gray-600 mt-0.5">Test your comprehension with an auto-graded MCQ test.</p>
                  </div>
                </div>
                <Link
                  href={`/student/quizzes/${activeQuizId}`}
                  className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-lg transition shadow-sm"
                >
                  Take Quiz
                </Link>
              </div>
