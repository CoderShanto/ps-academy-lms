...
const themes = {
  indigo: {
    ...
    footer: 'bg-slate-900 dark:bg-black',
    heroText: 'text-white',
    heroSubtext: 'text-gray-300',
    heroBadge: 'bg-white/10 border-white/20 text-white/90',
  },
  emerald: {
    name: 'Emerald',
    primary: '#059669',
...
export default function Home() {
  ...
  const [colorTheme, setColorTheme] = useState<'indigo' | 'emerald' | 'rose' | 'amber' | 'cyan' | 'purple'>('indigo');
  ...
  const changeColorTheme = (key: typeof colorTheme) => {
    setColorTheme(key);
  ...
  return (
    <div className={`flex flex-col min-h-screen transition-colors duration-500 ${isDark ? 'dark bg-slate-950' : 'bg-white'}`}>
      ...
      <div className="fixed top-24 right-6 z-50 flex flex-col items-end gap-3">
        ...
        {showThemePanel && (
          <div className={`p-4 rounded-2xl shadow-2xl border backdrop-blur-xl transition-all animate-in slide-in-from-right-4 ${
            isDark 
            ...
            <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-200 dark:border-slate-700">
              ...
              >
                <div className={`absolute top-1 w-5 h-5 rounded-full transition-all flex items-center justify-center ${
                  isDark 
                    ? 'left-8 bg-slate-900 text-yellow-400' 
            ...
            <div className="grid grid-cols-3 gap-2">
              {colorOptions.map((option) => (
                <button
                  key={option.key}
                  onClick={() => changeColorTheme(option.key as any)}
                  className={`group flex flex-col items-center gap-1 p-2 rounded-xl transition-all ${
                    colorTheme === option.key
                      ? isDark ? 'bg-slate-800 ring-2 ring-offset-2 ring-offset-slate-900' : 'bg-gray-100 ring-2 ring-offset-2'
                      : 'hover:bg-gray-100 dark:hover:bg-slate-800'
                  }`}
                  style={{ ringColor: option.color }}
                >
                  <div 
      ...
      {/* Hero Section */}
      <section 
        className={`relative bg-gradient-to-br ${theme.gradientDark} text-white overflow-hidden transition-all duration-700`}
      >
        ...
        <div className={`relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 lg:py-32 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="text-center">
            <div className={`inline-flex items-center gap-2 ${theme.heroBadge} backdrop-blur-sm rounded-full px-4 py-2 mb-8 transition-all duration-500`}>
            ...
            <p className={`mt-6 text-xl ${theme.heroSubtext} max-w-3xl mx-auto leading-relaxed transition-colors duration-500`}>
            ...
            <div className="mt-10 flex flex-col sm:flex-row justify-center gap-4">
              {user ? (
                ...
                <>
                  <Link
                    href="/register"
                    className={`inline-flex items-center justify-center ${theme.button} text-white font-semibold px-8 py-4 rounded-xl shadow-lg ${theme.buttonShadow} transition-all hover:scale-105`}
                  >
                    Start Free Trial
                  </Link>
                  <Link
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {stats.map((stat, index) => (
              <div 
                key={index}
                className={`text-center p-6 rounded-2xl ${stat.bgColor} border ${theme.border} hover:shadow-lg transition-all duration-300 hover:-translate-y-1`}
              >
                <stat.icon className={`h-8 w-8 ${stat.color} mx-auto mb-3 transition-colors duration-500`} />
                <div className={`text-3xl font-bold ${isDark ? 'text-white' : 'text-gray-900'} mb-1 transition-colors duration-500`}>{stat.value}</div>
                <div className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'} font-medium transition-colors duration-500`}>{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className={`py-24 ${isDark ? 'bg-slate-950' : 'bg-gray-50'} transition-colors duration-500`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className={`text-3xl sm:text-4xl font-bold ${isDark ? 'text-white' : 'text-gray-900'} mb-4 transition-colors duration-500`}>
              Why Organizations Choose PS Academy
            </h2>
            <p className={`text-lg ${isDark ? 'text-gray-400' : 'text-gray-600'} max-w-2xl mx-auto transition-colors duration-500`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {features.map((feature, index) => (
              <div 
                key={index}
                className={`group p-8 ${isDark ? 'bg-slate-900' : 'bg-white'} rounded-2xl border-2 ${feature.borderColor} ${theme.cardHover} hover:shadow-xl transition-all duration-300 hover:-translate-y-1`}
              >
                ...
                <h3 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'} mb-3 transition-colors duration-500`}>{feature.title}</h3>
                <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'} leading-relaxed transition-colors duration-500`}>{feature.description}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {testimonials.map((testimonial, index) => (
              <div 
                key={index}
                className={`p-8 ${isDark ? 'bg-slate-950 border-slate-800' : 'bg-gray-50 border-gray-200'} rounded-2xl border hover:shadow-lg transition-all duration-300`}
              >
                <Quote className={`h-10 w-10 ${theme.testimonialQuote} mb-4 opacity-50 transition-colors duration-500`} />
                <p className={`${isDark ? 'text-gray-300' : 'text-gray-700'} mb-6 leading-relaxed italic transition-colors duration-500`}>
                  "{testimonial.quote}"
                </p>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className={`font-semibold ${isDark ? 'text-white' : 'text-gray-900'} transition-colors duration-500`}>{testimonial.author}</div>
                    <div className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'} transition-colors duration-500`}>{testimonial.role}</div>
                    <div className={`text-sm ${theme.link} font-medium transition-colors duration-500`}>{testimonial.company}</div>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className={`py-24 bg-gradient-to-br ${theme.ctaGradient} text-white transition-all duration-700`}>
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <Link
              href="/register"
              className="inline-flex items-center justify-center bg-white font-semibold px-8 py-4 rounded-xl shadow-lg hover:shadow-xl transition-all hover:scale-105"
            ...
            <Link
              href="/demo"
              className="inline-flex items-center justify-center bg-white/20 hover:bg-white/30 backdrop-blur-sm border-2 border-white/30 text-white font-semibold px-8 py-4 rounded-xl transition-all"
            >
              Schedule a Demo
            </Link>
          </div>
      ...
      {/* Professional Footer */}
      <footer className={`${theme.footer} text-gray-300 py-12 border-t ${isDark ? 'border-slate-800' : 'border-slate-800'} transition-colors duration-500`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
