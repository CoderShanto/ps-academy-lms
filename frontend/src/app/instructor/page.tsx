...
function StudioContent() {
  ...
  const targetParamCourseId = searchParams.get('courseId');
  ...
  const fetchCourses = useCallback(async () => {
    try {
      let list: CourseItem[] = [];
      try {
        const { data } = await api.get('/courses?studio=true&populate=*');
        list = data.data || [];
      } catch {
        const { data } = await api.get('/courses?studio=true');
        list = data.data || [];
      }

      setCourses(list);

      // Select course from query parameter if available, otherwise first course
      if (list.length > 0) {
        if (targetParamCourseId && list.some((c) => String(c.documentId || c.id) === String(targetParamCourseId))) {
          setSelectedCourseId(targetParamCourseId);
        } else {
          setSelectedCourseId((prev) => {
            const exists = list.some((c) => String(c.documentId || c.id) === String(prev));
            return exists ? prev : list[0].documentId || list[0].id;
          });
        }
      } else {
        setSelectedCourseId('');
    ...
    try {
      if (editingLessonId) {
        ...
        const activeCourse = courses.find(
          (c) => String(c.id) === String(selectedCourseId) || String(c.documentId) === String(selectedCourseId)
        );
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">
          {roleName.includes('instructor') ? 'Instructor Studio' : 'Curriculum & Course Studio'}
        </h1>
        <p className="text-gray-600 mt-1 mb-6">
          {roleName.includes('instructor')
            ? 'Manage your courses, lessons, attached quizzes, and student progress'
        ...
        <div className="flex flex-wrap gap-3">
          ...
          >
            {roleName.includes('instructor') ? 'My Courses & Quizzes' : 'Course & Quiz Studio'}
          </Link>
          <Link
            href="/admin/progress"
          ...
          >
            {roleName.includes('instructor') ? 'My Students Progress' : 'View Student Progress'}
          </Link>
          {roleName.includes('admin') && (
            <Link
              href="/admin"
          ...
          {(roleName.includes('admin') || roleName.includes('manager') || roleName.includes('content')) && (
            <Link
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
            ...
            {courses.length === 0 ? (
              <p className="text-sm text-gray-500 py-4 text-center">No courses found. Create one above.</p>
            ) : (
            ...
            <form onSubmit={handleSaveLesson} className="space-y-5">
              <div>
                ...
                <select
                  ...
                  disabled={Boolean(editingLessonId) || courses.length === 0}
              ...
              <button
                type="submit"
                disabled={savingLesson || !selectedCourseId}
