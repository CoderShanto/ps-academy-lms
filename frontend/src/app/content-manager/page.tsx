...
import { PlusCircle, FileText, CheckCircle, Clock, Trash2, Edit3, X } from 'lucide-react';
...
export default function ContentManagerPage() {
  ...
  const handleStartEdit = (article: BlogPost) => {
    setEditingId(article.documentId || article.id);
    setTitle(article.title || '');
    setContent(article.content || '');
    setIsPublished(Boolean(article.publishedAt));
  ...
  const handleSubmitArticle = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        await api.put(`/blog-posts/${editingId}`, { data: payload });
      } else {
        // Create new
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-1 mb-6">Write, edit, publish, and delete academy articles with Draft vs Published states</p>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 h-fit">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold text-gray-900 flex items-center">
              {editingId ? (
                <>
                  <Edit3 className="h-5 w-5 mr-2 text-amber-600" /> Edit Article
                </>
              ) : (
                <>
                  <PlusCircle className="h-5 w-5 mr-2 text-indigo-600" /> Write New Article
                </>
              )}
            </h2>
          ...
          <form onSubmit={handleSubmitArticle} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. 10 Tips for Next.js Full Stack Apps"
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {articles.length === 0 ? (
            ...
            articles.map((art) => {
              ...
              const artId = art.documentId || art.id;
              const isBeingEdited = editingId === artId;

              return (
                <div
                  key={art.id}
                  className={`bg-white p-5 rounded-2xl shadow-sm border transition flex items-start justify-between gap-4 ${
                    isBeingEdited ? 'border-amber-400 ring-2 ring-amber-100' : 'border-gray-200'
                  }`}
                >
